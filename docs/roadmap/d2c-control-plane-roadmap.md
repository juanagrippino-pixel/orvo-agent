...
- JQL-lite and built-in operator views now support WorkItem projection fields including `project`, `issue_type`, `release_state`, `status_category`, `work_item_id`, and `assignee_ref`; they remain route/business-scoped projections and do not translate user input to SQL or persist custom saved views.
...
- JQL-lite additions for canonical fields (`project`, `status_category`, `release_state`, `work_item_id`, `assignee_ref`, and `issue_type`), with route-owned business scope.
