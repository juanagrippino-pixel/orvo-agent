...
- JQL-lite, facets, and built-in operator views now support WorkItem projection fields including `project`, `issue_type`, `release_state`, `status_category`, and `assignee_ref`; they remain route/business-scoped projections and do not translate user input to SQL or persist custom saved views.
...
- JQL-lite additions for canonical fields (`project`, `status_category`, `assignee_ref`, `issue_type`, and `release_state`), with route-owned business scope.
