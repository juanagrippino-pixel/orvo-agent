...
The HTTP layer for Tiendanube (a `POST /brain/reports/daily/tiendanube`
endpoint and a `run_tiendanube_daily_report_pipeline` runner) is not yet wired
into `server.py`. Until that lands, drive the adapter from a CLI script that
loads credentials from `ConnectorConfig.params`.
