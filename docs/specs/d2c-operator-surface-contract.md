...
- support the read-only `sla_status`, `service_record_type`, and `owner_status` query filters on the service-management endpoint so operators can open deterministic SLA, incident, request, problem, change, waiting-owner, or waiting-external queues without changing case lifecycle state; `total` counts the filtered queue and `unfiltered_total` preserves the full scoped case count;
