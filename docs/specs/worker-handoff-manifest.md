...
The git check verifies that the worktree path still exists or the branch is present locally/remotely, that `base_sha` and committed `head_sha` resolve to commits, that the manifest `status` matches the live worktree's dirty/clean state when the worktree still exists (`clean`, `review-ready`, and `merged` require no uncommitted changes; `dirty-blocked` requires them), and that `files_changed` exactly matches `git diff --name-only base_sha...head_sha`. Use it on specific manifests first because older durable manifests may contain human placeholders, intentionally dirty `head_sha: uncommitted` handoffs, or archived/abandoned worktrees whose status should be reviewed manually.
