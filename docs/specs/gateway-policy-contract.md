...
2. `operator_api.service_catalog.read` — read-only developer-platform service catalog projection; requires `internal:read`; no idempotency key.
3. `operator_api.case_action.mutate` — case lifecycle/comment/assignment actions; requires `case:action`; idempotency key required.
4. `runtime.force_run.mutate` — operator-triggered runtime execution; requires `runtime:execute`; idempotency key required.

These policies started as conventions and contract tests. Public manifests include `enforcement_state` so the registry does not overstate current gateway coverage. The current Python runtime now marks and enforces `operator_api.case_queue.read` before opening stores for the internal case queue route, marks/enforces `operator_api.service_catalog.read` before projecting the service catalog manifest, and marks/enforces `operator_api.case_action.mutate` before mutation, including the business-scoped idempotency-key requirement and denied-decision audit projection. `runtime.force_run.mutate` remains `contract_only` until its route middleware wiring lands and must preserve the public response envelope when promoted.
