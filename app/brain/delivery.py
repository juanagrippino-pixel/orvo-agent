...
Design:
    - WhatsAppDeliveryClient: thin wrapper over the WhatsApp Cloud API.
    - DeliveryTarget: validated phone + business_id.
    - DeliveryResult: structured success/failure output.
    - build_report_delivery_payload: constructs the API JSON body.
    - make_idempotency_key: deterministic key for deduplication.

Runtime credentials are read from env vars WHATSAPP_PHONE_ID and
WHATSAPP_TOKEN via WhatsAppDeliveryClient.from_env(). Tests inject a
mock http_client — no real network calls are made.
"""
