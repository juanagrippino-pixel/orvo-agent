...
def list_recently_reopened_cases(
    ...
    *,
    ...
    the workflow: once an operator reopens a resolved case via the
    ``reopen_case`` mutation, this projection surfaces it on the operator's
    "Just bounced back" panel so the team can route it before it goes stale.
    Only cases currently in ``open`` status are included; if the case is
    re-acknowledged or re-resolved after being reopened it moves to the
    recently-acknowledged or recently-resolved projection respectively. The
    reopen timestamp is taken from the latest ``case_reopened`` timeline event
    (cases may be reopened more than once), and rows are ordered by that
    timestamp DESC with ``case_id`` ASC as a deterministic tie-breaker.
    Strictly scoped per tenant; the projection reads timeline events directly
    from the case store, so it needs no ``now`` parameter.
    """

    parsed_limit = parse_limit(limit)
    reopened: list[tuple[datetime, str, OperationalCase, datetime]] = []
    for case in store.list_cases(business_id=business_id, status="open", limit=None):
        ...
        if latest_reopen_at is None:
            continue
        reopened.append((latest_reopen_at, case.case_id, case, latest_reopen_at))

    # Most recently reopened first; tie-break by case_id ASC for deterministic order.
    ...
    cases_payload = [
        ...
        for _sort_at, _case_id, case, reopened_at in limited
    ]
    return redact_secrets(
        {
            "business_id": business_id,
...
def summarize_case_resolution_latency_histogram_by_severity(
    store: OperationalCaseStore, *, business_id: str
) -> dict[str, Any]:
    """Severity-split deterministic histogram of time-to-resolve.

    Mirrors :func:`summarize_case_resolution_latency_histogram_by_priority_bracket`
    but groups each resolution-latency bucket by case severity
    (info / warning / critical) instead of priority bracket, matching the
    attribution used by :func:`summarize_case_queue_aging_by_severity` and
    :func:`summarize_case_queue_stagnation_by_severity`. Operator surfaces use
    it to spot when long-tail resolution time is concentrated in critical
    cases even though the overall median, case_type, entity_kind,
    source-connector and priority-bracket distributions look healthy. Open and
    acknowledged-only cases are excluded; ``resolved_total`` only counts cases
    that reached ``resolved`` state. Strictly scoped per tenant.
    """
...
def summarize_case_acknowledgment_latency_histogram_by_severity(
    ...
    """Severity-split deterministic histogram of time-to-acknowledge.

    Mirrors :func:`summarize_case_acknowledgment_latency_histogram_by_priority_bracket`
    but groups each acknowledgment-latency bucket by case severity
    (info / warning / critical) instead of priority bracket, matching the
    attribution used by :func:`summarize_case_resolution_latency_histogram_by_severity`,
    :func:`summarize_case_queue_aging_by_severity` and
    :func:`summarize_case_queue_stagnation_by_severity`. Operator surfaces
    use it to spot when long-tail first-touch latency is concentrated in
    critical cases even though the overall median, case_type, entity_kind,
    source-connector and priority-bracket distributions look healthy.
    Resolved cases are still counted (they were acknowledged at some
    point); only open-only cases without ``acknowledged_at`` are excluded.
    Strictly scoped per tenant.
    """
...
def summarize_case_handling_latency_histogram_by_severity(
    ...
    Mirrors :func:`summarize_case_handling_latency_histogram_by_priority_bracket`
    on the hands-on leg of the workflow (``resolved_at - acknowledged_at``),
    grouping each handling-latency bucket by case severity. Open and
    acknowledged-only cases are excluded; ``handled_total`` only counts cases
    that reached ``resolved`` state. Strictly scoped per tenant.
    """
