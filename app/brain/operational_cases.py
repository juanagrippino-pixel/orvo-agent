...
# Only case families with semantic-registry evidence contracts are eligible for
# deterministic insight -> OperationalCase promotion. Additional literals can
# remain internal/deferred catalog targets until CASE_FAMILY_METRICS is expanded
# with the required evidence/freshness policy.
DETECTABLE_CASE_TYPES: frozenset[str] = frozenset(CASE_FAMILY_METRICS)
...
def _case_type_for_insight(insight: Insight) -> OperationalCaseType | None:
    ...
    if candidate is None or candidate not in DETECTABLE_CASE_TYPES:
        return None
    return candidate
