"""Tests for the case-type-split resolution-latency histogram projection."""
...
def test_resolution_latency_histogram_by_case_type_returns_empty_when_no_cases():
    store = InMemoryOperationalCaseStore()

    result = summarize_case_resolution_latency_histogram_by_case_type(
    ...
    assert result == {
        ...
        "by_resolution_bucket_case_type": {bucket: {} for bucket in _empty_buckets()},
        "fastest_resolved": None,
        "slowest_resolved": None,
    }


def test_resolution_latency_histogram_by_case_type_buckets_by_case_type():
    store = InMemoryOperationalCaseStore()
    ...
    assert result["by_resolution_bucket_case_type"] == {
...
def test_resolution_latency_histogram_by_case_type_excludes_open_and_acknowledged_cases():
    store = InMemoryOperationalCaseStore()
    ...
    assert result["by_resolution_bucket_case_type"]["under_6h"] == {
...
def test_resolution_latency_histogram_by_case_type_is_scoped_per_business():
    store = InMemoryOperationalCaseStore()
    ...
    assert "data_stale" not in result["by_resolution_bucket_case_type"]["over_7d"]


def test_resolution_latency_histogram_by_case_type_groups_multiple_cases_per_bucket():
    store = InMemoryOperationalCaseStore()
    opened_at = NOW - timedelta(days=30)
    ...
    assert result["by_resolution_bucket_case_type"]["over_7d"] == {"data_stale": 1}
