"""Tests for the severity-split case-queue summary projection.

``summarize_case_queue`` already exposes lifecycle counts by status, severity,
and case type. This projection mirrors it but groups every count by case
``severity`` (info / warning / critical), matching the attribution used by
:func:`summarize_case_queue_aging_by_severity`,
:func:`summarize_case_queue_stagnation_by_severity`, and
:func:`summarize_case_workflow_throughput_by_severity`. Operator surfaces use
it to spot when a single severity (e.g. ``critical``) dominates the actionable
backlog or carries the bulk of the degraded-evidence cases even when the
...
from app.brain.operator_api import summarize_case_queue_by_severity
...
def _seed_mixed_queue() -> InMemoryOperationalCaseStore:
    ...
    # Open info sales drop in a different entity scope (different dedupe).
    store.upsert_detection(
        _detection(
            case_type="sales_drop",
    ...
    # Resolved warning data_stale: counted in total but not actionable.
...
def test_summarize_case_queue_by_severity_groups_lifecycle_counts():
    store = _seed_mixed_queue()

    result = summarize_case_queue_by_severity(store, business_id="artemea")

    assert result["business_id"] == "artemea"
    assert result["total"] == 4
    # critical: stockout(ack); warning: sales_drop(open, degraded) + data_stale(resolved);
    # info: sales_drop(open)
    assert result["totals_by_severity"] == {
        "critical": 1,
        "warning": 2,
        "info": 1,
    }
    # Actionable = open + acknowledged: stockout(critical), sales_drop(warning), sales_drop(info)
    assert result["actionable_total"] == 3
    assert result["actionable_by_severity"] == {
        "critical": 1,
        "warning": 1,
        "info": 1,
    }
    # Only the open warning sales_drop has degraded evidence.
    assert result["actionable_degraded_by_severity"] == {"warning": 1}


def test_summarize_case_queue_by_severity_is_scoped_per_business():
    store = _seed_mixed_queue()

    result = summarize_case_queue_by_severity(store, business_id="other-shop")

    assert result["business_id"] == "other-shop"
    assert result["total"] == 1
    assert result["totals_by_severity"] == {"critical": 1}
    assert result["actionable_total"] == 1
    assert result["actionable_by_severity"] == {"critical": 1}
    assert result["actionable_degraded_by_severity"] == {}


def test_summarize_case_queue_by_severity_returns_empty_when_no_cases():
    store = InMemoryOperationalCaseStore()
    ...
    assert result == {
        ...
        "actionable_degraded_by_severity": {},
    }


def test_summarize_case_queue_by_severity_excludes_resolved_from_actionable():
    store = InMemoryOperationalCaseStore()
    # Critical open case stays actionable.
    store.upsert_detection(
        _detection(case_type="stockout_risk", severity="critical", run_id="run-open"),
        detected_at=_utc(8),
    )
    # Critical resolved case still counts toward total but not actionable.
    ...
    assert result["totals_by_severity"] == {"critical": 2}
    assert result["actionable_total"] == 1
    assert result["actionable_by_severity"] == {"critical": 1}
    assert result["actionable_degraded_by_severity"] == {}
