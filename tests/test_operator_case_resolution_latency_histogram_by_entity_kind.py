"""Tests for the entity-kind-split resolution-latency histogram projection."""
...
def test_resolution_latency_histogram_by_entity_kind_returns_empty_when_no_cases():
    store = InMemoryOperationalCaseStore()

    result = summarize_case_resolution_latency_histogram_by_entity_kind(
    ...
    assert result == {
        ...
        "by_resolution_bucket_entity_kind": {bucket: {} for bucket in _empty_buckets()},
        "fastest_resolved": None,
        "slowest_resolved": None,
    }


def test_resolution_latency_histogram_by_entity_kind_buckets_by_entity_kind():
    store = InMemoryOperationalCaseStore()
    ...
    for run_id, case_type, dedupe_suffix, entity_scope, ttr in schedule:
        _resolve(
            store,
    ...
    assert result["by_resolution_bucket_entity_kind"] == {
        "under_1h": {"product": 1},
        "under_6h": {"channel": 1},
        "under_24h": {"channel": 1},
...
def test_resolution_latency_histogram_by_entity_kind_excludes_open_and_acknowledged_cases():
    store = InMemoryOperationalCaseStore()
    ...
    assert result["by_resolution_bucket_entity_kind"]["under_6h"] == {
        "conversation": 1,
    }
...
def test_resolution_latency_histogram_by_entity_kind_is_scoped_per_business():
    store = InMemoryOperationalCaseStore()
    ...
    assert "business" not in result["by_resolution_bucket_entity_kind"]["over_7d"]


def test_resolution_latency_histogram_by_entity_kind_groups_multiple_cases_per_bucket():
    store = InMemoryOperationalCaseStore()
    opened_at = NOW - timedelta(days=30)
    ...
    assert result["by_resolution_bucket_entity_kind"]["over_7d"] == {"business": 1}


def test_resolution_latency_histogram_by_entity_kind_buckets_missing_kind_as_unknown():
    store = InMemoryOperationalCaseStore()
    ...
    _resolve(
        ...
        opened_at=opened_at,
        resolved_at=opened_at + timedelta(hours=3),
    )
    ...
    assert result["resolved_total"] == 1
    assert result["by_resolution_bucket"]["under_6h"] == 1
    assert result["by_resolution_bucket_entity_kind"]["under_6h"] == {"unknown": 1}
