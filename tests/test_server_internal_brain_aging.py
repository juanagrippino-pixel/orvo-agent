"""HTTP endpoint tests for the case queue aging endpoint."""
...
def test_case_aging_returns_empty_summary_when_no_cases(_isolate_db):
    from server import app

    client = app.test_client()
    response = client.get(
        "/internal/brain/businesses/artemea/cases/aging",
        headers=AUTH,
    )
    ...
    assert all(count == 0 for count in data["by_age_bucket"].values())
    assert data["oldest_actionable"] is None


def test_case_aging_returns_age_buckets_for_open_cases(_isolate_db):
    from server import app

    _seed_open_case(_isolate_db, opened_hours_ago=2)
...
def test_case_aging_scopes_to_business_id(_isolate_db):
    ...
    client = app.test_client()
    response = client.get(
        "/internal/brain/businesses/other-biz/cases/aging",
        headers=AUTH,
    )
...
def test_case_aging_rejects_unauthorized_request(_isolate_db):
    from server import app
...
def test_case_aging_returns_503_when_auth_not_configured(_isolate_db, monkeypatch):
    from server import app
