"""Tests for the priority-bracket-split case-queue aging projection.
...
each age bucket by deterministic priority bracket (low / medium / high)
derived from ``case.priority_score`` instead of source connector. Operator
surfaces use it to spot when the in-flight backlog is skewed toward
high-priority work even when the overall age distribution looks healthy.
"""
...
def test_summarize_case_queue_aging_by_priority_bracket_returns_empty_summary_when_no_cases():
    store = InMemoryOperationalCaseStore()

    result = summarize_case_queue_aging_by_priority_bracket(
    ...
    assert result == {
        ...
        "by_age_bucket_priority_bracket": {bucket: {} for bucket in _empty_buckets()},
        "oldest_actionable": None,
    }


def test_summarize_case_queue_aging_by_priority_bracket_splits_actionable_by_bracket_per_bucket():
    store = InMemoryOperationalCaseStore()
    ...
    assert result["by_age_bucket_priority_bracket"] == {
        "under_1h": {"high": 1},
        "under_6h": {"medium": 2},
        "under_24h": {"low": 1},
        "under_7d": {"medium": 1},
...
def test_summarize_case_queue_aging_by_priority_bracket_buckets_score_boundaries():
    ...
    for run_id, dedupe_suffix, priority, _expected in schedule:
        store.upsert_detection(
            ...
            detected_at=NOW - timedelta(hours=2),  # all in under_6h
        )
    ...
    assert result["actionable_total"] == 6
    assert result["by_age_bucket"]["under_6h"] == 6
    assert result["by_age_bucket_priority_bracket"]["under_6h"] == {
        "low": 2,
        "medium": 2,
        "high": 2,
    }
...
def test_summarize_case_queue_aging_by_priority_bracket_excludes_resolved_cases():
    store = InMemoryOperationalCaseStore()
...
def test_summarize_case_queue_aging_by_priority_bracket_includes_acknowledged_as_actionable():
    store = InMemoryOperationalCaseStore()
    ...
    assert result["by_age_bucket_priority_bracket"]["under_24h"] == {"medium": 1}


def test_summarize_case_queue_aging_by_priority_bracket_is_scoped_per_business():
    store = InMemoryOperationalCaseStore()
    store.upsert_detection(
        _detection(
            business_id="artemea",
...
def test_summarize_case_queue_aging_by_priority_bracket_uses_current_utc_when_now_not_provided(monkeypatch):
    store = InMemoryOperationalCaseStore()
    store.upsert_detection(
    ...
    result = summarize_case_queue_aging_by_priority_bracket(store, business_id="artemea")

    assert result["now"] == "2026-05-26T12:00:00+00:00"
    assert result["actionable_total"] == 1
    assert result["by_age_bucket"]["under_1h"] == 1
    assert result["by_age_bucket_priority_bracket"]["under_1h"] == {"high": 1}


def test_summarize_case_queue_aging_by_priority_bracket_rejects_naive_now():
    store = InMemoryOperationalCaseStore()
