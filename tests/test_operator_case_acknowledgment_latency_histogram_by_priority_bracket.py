"""Tests for the priority-bracket-split acknowledgment-latency histogram projection.

Acknowledgment latency = ``acknowledged_at - opened_at`` (queue-wait leg).
This projection mirrors
:func:`summarize_case_acknowledgment_latency_histogram_by_source_connector`
but groups each bucket by deterministic priority bracket
(low / medium / high) derived from ``case.priority_score`` instead of
source connector. Operator surfaces use it to spot when long-tail
first-touch latency is concentrated in high-priority cases even though
the overall median, case_type, entity_kind and source-connector
...
def test_acknowledgment_latency_histogram_by_priority_bracket_returns_empty_when_no_cases():
    store = InMemoryOperationalCaseStore()

    result = summarize_case_acknowledgment_latency_histogram_by_priority_bracket(
    ...
    assert result == {
        ...
        "by_acknowledgment_bucket_priority_bracket": {
...
def test_acknowledgment_latency_histogram_by_priority_bracket_buckets_by_priority_bracket():
    store = InMemoryOperationalCaseStore()
    opened_at = NOW - timedelta(days=30)

    # (run_id, case_type, dedupe_suffix, priority_score, tta, expected_bracket)
    schedule = [
        (
            "a",
            "stockout_risk",
    ...
    for run_id, case_type, dedupe_suffix, priority, tta in schedule:
        _acknowledge(
            store,
    ...
    assert result["by_acknowledgment_bucket_priority_bracket"] == {
        "under_1h": {"high": 1},
        "under_6h": {"medium": 1},
        "under_24h": {"medium": 1},
        "under_7d": {"low": 1},
...
def test_acknowledgment_latency_histogram_by_priority_bracket_buckets_score_boundaries():
    ...
    for run_id, dedupe_suffix, priority, _expected in schedule:
        _acknowledge(
            ...
            opened_at=opened_at,
            acknowledged_at=opened_at + timedelta(hours=2),  # all in under_6h
        )
    ...
    assert result["acknowledged_total"] == 6
    assert result["by_acknowledgment_bucket"]["under_6h"] == 6
    assert result["by_acknowledgment_bucket_priority_bracket"]["under_6h"] == {
        "low": 2,
        "medium": 2,
        "high": 2,
    }
...
def test_acknowledgment_latency_histogram_by_priority_bracket_excludes_open_only_cases():
    store = InMemoryOperationalCaseStore()
    ...
    # Acknowledged -> counted; 1h tta -> under_6h, priority 65 -> medium.
    ack_id = _acknowledge(
        store,
        detection=_detection(
    ...
    assert result["by_acknowledgment_bucket_priority_bracket"]["under_6h"] == {"medium": 1}
    assert result["fastest_acknowledged"]["case_id"] == ack_id
    assert result["slowest_acknowledged"]["case_id"] == ack_id


def test_acknowledgment_latency_histogram_by_priority_bracket_counts_resolved_cases():
    # Resolved cases were acknowledged at some point — they should still
    # appear in the acknowledgment-latency histogram. Symmetric with the
    # other ack-latency split projections.
    store = InMemoryOperationalCaseStore()
    ...
    store.transition_case(
        ...
        actor_type="operator",
        actor_ref="operator@example.com",
        transitioned_at=opened_at + timedelta(hours=3),  # under_6h
    )
    store.transition_case(
        case.case_id,
        status="resolved",
    ...
    assert result["acknowledged_total"] == 1
    assert result["by_acknowledgment_bucket"]["under_6h"] == 1
    assert result["by_acknowledgment_bucket_priority_bracket"]["under_6h"] == {"high": 1}


def test_acknowledgment_latency_histogram_by_priority_bracket_is_scoped_per_business():
    store = InMemoryOperationalCaseStore()
    ...
    _acknowledge(
        ...
        opened_at=opened_at,
        acknowledged_at=opened_at + timedelta(hours=2),  # under_6h -> high
    )
    _acknowledge(
        store,
        detection=_detection(
        ...
        opened_at=opened_at,
        acknowledged_at=opened_at + timedelta(days=9),  # over_7d -> low
    )
    ...
    assert result["by_acknowledgment_bucket_priority_bracket"]["under_6h"] == {"high": 1}
    assert "low" not in result["by_acknowledgment_bucket_priority_bracket"]["over_7d"]


def test_acknowledgment_latency_histogram_by_priority_bracket_groups_multiple_cases_per_bucket():
    store = InMemoryOperationalCaseStore()
    opened_at = NOW - timedelta(days=30)
    ...
    assert result["by_acknowledgment_bucket_priority_bracket"]["over_7d"] == {"low": 1}


def test_acknowledgment_latency_histogram_by_priority_bracket_clamps_non_positive_latency_to_zero():
    ...
    assert result["by_acknowledgment_bucket_priority_bracket"]["under_1h"] == {"high": 1}
