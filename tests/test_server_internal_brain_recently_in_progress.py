"""HTTP endpoint tests for the recently-in-progress case listing endpoint."""
...
def _detection(
    *,
    ...
    return OperationalCaseDetection(
        ...
        title="Recently in-progress case token=raw_secret_title",
        severity=severity,
...
def _transition_case(
    ...
    *,
    ...
    store.transition_case(
        ...
        reason="done" if status in {"resolved", "dismissed"} else None,
        transitioned_at=datetime.now(timezone.utc) - timedelta(minutes=transitioned_minutes_ago),
    )
    conn.close()


def test_recently_in_progress_returns_scoped_cases_ordered_newest_first(_isolate_db):
    ...
    open_case = _seed_open_case(
        ...
        opened_hours_ago=1,
        run_id="run-open",
        dedupe_suffix="recent/progress/open",
    )
    acknowledged = _seed_open_case(
        _isolate_db,
        opened_hours_ago=2,
        run_id="run-ack",
        dedupe_suffix="recent/progress/ack",
    )
    ...
    resolved = _seed_open_case(
        ...
        opened_hours_ago=8,
        run_id="run-resolved",
        dedupe_suffix="recent/progress/resolved",
    )

    _transition_case(_isolate_db, older, status="in_progress", transitioned_minutes_ago=30)
    _transition_case(_isolate_db, newest, status="in_progress", transitioned_minutes_ago=5)
    _transition_case(_isolate_db, acknowledged, status="acknowledged", transitioned_minutes_ago=10)
    _transition_case(_isolate_db, other_business, status="in_progress", transitioned_minutes_ago=1)
    _transition_case(_isolate_db, resolved, status="in_progress", transitioned_minutes_ago=2)
    ...
    assert "raw_secret_title" not in raw_body
...
def test_recently_in_progress_requires_internal_auth(_isolate_db):
    from server import app

    client = app.test_client()
    response = client.get(
        "/internal/brain/businesses/artemea/cases/recently-in-progress",
    )
