"""Tests for the deterministic recently-opened cases projection."""
...
def test_returns_empty_when_no_cases():
    ...
    result = list_recently_opened_cases(store, business_id="artemea")
...
def test_orders_most_recently_opened_first():
    store = InMemoryOperationalCaseStore()
    ...
    for run_id, case_type, dedupe_suffix, severity, since_open in schedule:
        opened_at = NOW - since_open
    ...
    assert first["opened_at"].startswith("2026-05-26T10:00:00")
...
def test_respects_limit():
    ...
    cases_by_run: dict[str, str] = {}
    for index, (run_id, since_open) in enumerate(schedule):
        case = store.upsert_detection(
            _detection(
            ...
            detected_at=NOW - since_open,
        )
        cases_by_run[run_id] = case.case_id

    result = list_recently_opened_cases(store, business_id="artemea", limit="2")
...
def test_excludes_acknowledged_and_resolved_cases():
    store = InMemoryOperationalCaseStore()
    ...
    # Acknowledged — excluded; recently_acknowledged owns it.
    ...
    _acknowledge(store, ack.case_id, acknowledged_at=NOW - timedelta(hours=3))

    # Resolved — excluded; recently_resolved owns it.
...
def test_tiebreaks_on_case_id():
    ...
    assert all(entry["opened_at"] == opened_at.isoformat() for entry in result["cases"])
