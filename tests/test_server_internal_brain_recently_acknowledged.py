...
def test_recently_acknowledged_returns_scoped_in_flight_cases_ordered_newest_first(_isolate_db):
    ...
    _transition_case(_isolate_db, older, status="acknowledged", transitioned_minutes_ago=30)
    _transition_case(_isolate_db, newest, status="acknowledged", transitioned_minutes_ago=5)
    _transition_case(_isolate_db, other_business, status="acknowledged", transitioned_minutes_ago=1)
...
def test_recently_acknowledged_requires_internal_auth(_isolate_db):
    from server import app

    client = app.test_client()
    response = client.get(
        "/internal/brain/businesses/artemea/cases/recently-acknowledged",
    )
