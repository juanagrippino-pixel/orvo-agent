...
def _dismiss_case(db_path, case_id: str, *, dismissed_hours_ago: int = 0) -> None:
    ...
    store.transition_case(
        ...
        actor_type="operator",
        actor_ref="operator:juan",
        reason="False positive during operator follow-up.",
...
def test_recently_dismissed_returns_scoped_terminal_cases_ordered_newest_first(_isolate_db):
    ...
    reopened = _seed_open_case(
        _isolate_db,
        opened_hours_ago=8,
        run_id="run-dismissed-reopened",
        dedupe_suffix="recent/dismissed/reopened",
    )
    _dismiss_case(_isolate_db, reopened, dismissed_hours_ago=2)
    _seed_open_case(
        _isolate_db,
        opened_hours_ago=0,
        run_id="run-dismissed-reopened-2",
        dedupe_suffix="recent/dismissed/reopened",
    )
    other_business = _seed_open_case(
        _isolate_db,
