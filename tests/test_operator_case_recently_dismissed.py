...
def test_list_recently_dismissed_cases_returns_empty_when_no_cases():
    store = InMemoryOperationalCaseStore()
...
def test_list_recently_dismissed_cases_orders_most_recent_first():
    store = InMemoryOperationalCaseStore()
    ...
    for run_id, case_type, dedupe_suffix, severity, since_dismissed in schedule:
        opened_at = NOW - since_dismissed - timedelta(hours=12)
        dismissed_at = NOW - since_dismissed
    ...
    assert result["dismissed_total"] == 4
    ...
    assert first["dismissed_at"].startswith("2026-05-26T10:00:00")
    assert first["dismissal_seconds"] == int(timedelta(hours=12).total_seconds())


def test_list_recently_dismissed_cases_respects_limit():
    store = InMemoryOperationalCaseStore()
    ...
    cases_by_run: dict[str, str] = {}
    for index, (run_id, since_dismissed) in enumerate(schedule):
        opened_at = NOW - since_dismissed - timedelta(hours=6)
        case = store.upsert_detection(
            _detection(
    ...
    result = list_recently_dismissed_cases(store, business_id="artemea", limit="2")
...
def test_list_recently_dismissed_cases_excludes_non_terminal_or_reopened_cases():
    store = InMemoryOperationalCaseStore()
    ...
    _dismiss(store, dismissed.case_id, dismissed_at=NOW - timedelta(hours=2))
    reopened = store.upsert_detection(
        _detection(
            case_type="fulfillment_backlog",
            dedupe_suffix="fulfillment_backlog/business/warehouse/commerce.fulfillment/daily",
            severity="warning",
            run_id="run-reopened",
        ),
        detected_at=NOW - timedelta(days=2),
    )
    _dismiss(store, reopened.case_id, dismissed_at=NOW - timedelta(days=1))
    store.upsert_detection(
        _detection(
            case_type="fulfillment_backlog",
            dedupe_suffix="fulfillment_backlog/business/warehouse/commerce.fulfillment/daily",
            severity="warning",
            run_id="run-reopened-2",
        ),
    ...
    assert result["cases"][0]["status"] == "dismissed"


def test_list_recently_dismissed_cases_is_scoped_per_business():
    store = InMemoryOperationalCaseStore()
    mine = store.upsert_detection(
        _detection(business_id="artemea", run_id="run-mine"),
        detected_at=NOW - timedelta(days=1),
    )
    _dismiss(store, mine.case_id, dismissed_at=NOW - timedelta(hours=4))
    other = store.upsert_detection(
        _detection(business_id="other-shop", run_id="run-other"),
        detected_at=NOW - timedelta(days=2),
    )
    _dismiss(store, other.case_id, dismissed_at=NOW - timedelta(hours=1))
...
def test_list_recently_dismissed_cases_tiebreaks_on_case_id():
    store = InMemoryOperationalCaseStore()
    ...
    assert all(
        entry["dismissed_at"] == dismissed_at.isoformat()
        for entry in result["cases"]
    )


def test_list_recently_dismissed_cases_rejects_invalid_limit():
    store = InMemoryOperationalCaseStore()
