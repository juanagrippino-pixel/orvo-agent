"""Tests for the entity-kind-split case-queue stagnation (idleness) projection."""
...
def _detection(
    *,
    ...
    priority: int = 100,
    run_id: str = "run-stagnation-entity-1",
...
def test_summarize_case_queue_stagnation_by_entity_kind_returns_empty_summary_when_no_cases():
    store = InMemoryOperationalCaseStore()

    result = summarize_case_queue_stagnation_by_entity_kind(store, business_id="artemea", now=NOW)
    ...
    assert result == {
        ...
        "by_idle_bucket_entity_kind": {bucket: {} for bucket in _empty_buckets()},
        "most_stalled_actionable": None,
    }


def test_summarize_case_queue_stagnation_by_entity_kind_splits_actionable_by_entity_kind_per_bucket():
    store = InMemoryOperationalCaseStore()
    ...
    schedule = [
        # (run_id, case_type, dedupe_suffix, entity_kind, age_offset)
        ("a", "stockout_risk", "stockout_risk/product/sku-a/commerce.inventory/daily", "product", timedelta(minutes=30)),    # under_1h
        ("b", "stockout_risk", "stockout_risk/product/sku-b/commerce.inventory/daily", "product", timedelta(hours=3)),       # under_6h
        ("c", "sales_drop", "sales_drop/channel/all/commerce.revenue/daily", "channel", timedelta(hours=3)),                  # under_6h
        ("d", "sales_drop", "sales_drop/channel/meta_ads/commerce.revenue/daily", "channel", timedelta(hours=10)),            # under_24h
        ("e", "unanswered_conversations", "unanswered_conversations/channel/whatsapp/support.conversations/daily", "channel", timedelta(days=2)),  # under_7d
        ("f", "data_stale", "data_stale/business/monitored/observability.feeds/daily", "business", timedelta(days=10)),       # over_7d
    ]
    for run_id, case_type, dedupe_suffix, entity_kind, age in schedule:
        entity_id = dedupe_suffix.split("/")[2]
        store.upsert_detection(
            _detection(
                ...
                priority=70,
                run_id=f"run-{run_id}",
                entity_scope={"kind": entity_kind, "id": entity_id, "label": entity_id},
            ),
            detected_at=NOW - age,
        )
    ...
    assert result["by_idle_bucket_entity_kind"] == {
        "under_1h": {"product": 1},
        "under_6h": {"product": 1, "channel": 1},
        "under_24h": {"channel": 1},
        "under_7d": {"channel": 1},
        "over_7d": {"business": 1},
    }
...
def test_summarize_case_queue_stagnation_by_entity_kind_buckets_missing_kind_as_unknown():
    store = InMemoryOperationalCaseStore()
    store.upsert_detection(
        _detection(
    ...
    assert result["by_idle_bucket_entity_kind"]["under_6h"] == {"unknown": 1}


def test_summarize_case_queue_stagnation_by_entity_kind_uses_updated_at_not_opened_at():
    ...
    # Entity-kind splits track idleness bucket, not age.
    assert result["by_idle_bucket_entity_kind"]["under_1h"] == {"product": 1}
    assert result["by_idle_bucket_entity_kind"]["under_7d"] == {"channel": 1}
    most_stalled = result["most_stalled_actionable"]
...
def test_summarize_case_queue_stagnation_by_entity_kind_excludes_resolved_cases():
    store = InMemoryOperationalCaseStore()
...
def test_summarize_case_queue_stagnation_by_entity_kind_includes_acknowledged_as_actionable():
    store = InMemoryOperationalCaseStore()
    ...
    assert result["by_idle_bucket_entity_kind"]["under_6h"] == {"product": 1}
    most_stalled = result["most_stalled_actionable"]
...
def test_summarize_case_queue_stagnation_by_entity_kind_is_scoped_per_business():
    store = InMemoryOperationalCaseStore()
    store.upsert_detection(
        _detection(
            business_id="artemea",
...
def test_summarize_case_queue_stagnation_by_entity_kind_uses_current_utc_when_now_not_provided(monkeypatch):
    store = InMemoryOperationalCaseStore()
    store.upsert_detection(
    ...
    result = summarize_case_queue_stagnation_by_entity_kind(store, business_id="artemea")
...
def test_summarize_case_queue_stagnation_by_entity_kind_rejects_naive_now():
    store = InMemoryOperationalCaseStore()
