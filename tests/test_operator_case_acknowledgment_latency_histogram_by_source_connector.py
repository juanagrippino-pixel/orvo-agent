"""Tests for the source-connector-split acknowledgment-latency histogram projection."""
...
def test_acknowledgment_latency_histogram_by_source_connector_returns_empty_when_no_cases():
    store = InMemoryOperationalCaseStore()

    result = summarize_case_acknowledgment_latency_histogram_by_source_connector(
    ...
    assert result == {
        ...
        "by_acknowledgment_bucket_source_connector": {
...
def test_acknowledgment_latency_histogram_by_source_connector_buckets_by_source():
    store = InMemoryOperationalCaseStore()
    opened_at = NOW - timedelta(days=30)

    schedule = [
        # (run_id, case_type, dedupe_suffix, source, tta)
        (
            "a",
            "stockout_risk",
    ...
    for run_id, case_type, dedupe_suffix, source, tta in schedule:
        _acknowledge(
            store,
    ...
    assert result["by_acknowledgment_bucket_source_connector"] == {
...
def test_acknowledgment_latency_histogram_by_source_connector_excludes_open_only_cases():
    store = InMemoryOperationalCaseStore()
    ...
    assert result["by_acknowledgment_bucket_source_connector"]["under_6h"] == {
        "google_sheets": 1,
    }
...
def test_acknowledgment_latency_histogram_by_source_connector_includes_resolved_cases():
    # Resolved cases were acknowledged at some point, so they must still
    # contribute to the histogram — mirroring the entity-kind and case-type
    # acknowledgment-latency splits.
    store = InMemoryOperationalCaseStore()
    opened_at = NOW - timedelta(days=3)
...
def test_acknowledgment_latency_histogram_by_source_connector_is_scoped_per_business():
    store = InMemoryOperationalCaseStore()
    ...
    assert (
        "google_sheets"
        not in result["by_acknowledgment_bucket_source_connector"]["over_7d"]
    )


def test_acknowledgment_latency_histogram_by_source_connector_groups_multiple_cases_per_bucket():
    store = InMemoryOperationalCaseStore()
    opened_at = NOW - timedelta(days=30)
    ...
    assert result["by_acknowledgment_bucket_source_connector"]["over_7d"] == {"csv": 1}


def test_acknowledgment_latency_histogram_by_source_connector_buckets_missing_source_as_unknown():
    # A detection with no evidence_refs leaves the case with zero evidence
    # snapshots, so _source_connectors returns []. Such acknowledged cases must
    ...
    assert result["acknowledged_total"] == 1
    assert result["by_acknowledgment_bucket"]["under_6h"] == 1
    assert result["by_acknowledgment_bucket_source_connector"]["under_6h"] == {"unknown": 1}


def test_acknowledgment_latency_histogram_by_source_connector_uses_first_sorted_source_for_multi_source_cases():
    ...
    # summarize_case_workflow_throughput_by_source_connector, and
    # summarize_case_resolution_latency_histogram_by_source_connector — so
    # bucket counts stay equal to the count of acknowledged cases without
    # double-counting.
    store = InMemoryOperationalCaseStore()
    opened_at = NOW - timedelta(days=2)
