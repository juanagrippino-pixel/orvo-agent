...
def test_force_report_runs_all_enabled_supported_connectors_and_dispatches_once():
    delivery = MagicMock()
    delivery.send_text.return_value = DeliveryResult(success=True, message_id="dry-run", error=None)
    ...
    metrics = {metric.key: metric.value for metric in result.report.metrics}
    assert metrics["revenue_today"] == pytest.approx(1000.0)
    ...
    assert "Publicidad" in sent_text
