...
def test_force_report_merges_all_enabled_daily_connectors_and_dispatches_once():
    delivery = MagicMock()
    delivery.send_text.return_value = DeliveryResult(success=True, message_id="dry-run", error=None)
    sheets_service_factory = MagicMock(side_effect=AssertionError("google sheets should not be loaded for tiendanube + meta_ads"))
