...
def test_force_report_merges_tiendanube_and_meta_ads_for_single_artemea_send():
    delivery = MagicMock()
    delivery.send_text.return_value = DeliveryResult(success=True, message_id="dry-run", error=None)
    ...
    result = reports_script.run_forced_report(
        ...
        tiendanube_http_client=FakeTiendanubeHTTPClient(),
        meta_ads_http_client=FakeMetaAdsHTTPClient(),
        mercadolibre_http_client=MagicMock(side_effect=AssertionError("mercadolibre must not be called")),
    )

    metrics = {metric.key: metric for metric in result.report.metrics}
    assert metrics["revenue_today"].value == 1000.0
    assert metrics["orders_today"].value == 1
    assert metrics["ad_spend_today"].value == pytest.approx(725.25)
    assert {ev.source for metric in result.report.metrics for ev in metric.evidence} >= {"tiendanube", "meta_ads"}
    assert result.dispatch.status == "sent"
