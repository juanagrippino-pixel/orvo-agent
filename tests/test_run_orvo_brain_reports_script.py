...
def make_artemea_hito0_business() -> BusinessConfig:
    return BusinessConfig(
        business_id="artemea",
...
def test_force_report_merges_all_enabled_hito0_connectors_before_one_dispatch():
    delivery = MagicMock()
    delivery.send_text.return_value = DeliveryResult(success=True, message_id="dry-run", error=None)
    sheets_service_factory = MagicMock(side_effect=AssertionError("google sheets should not be loaded without a sheets connector"))

    result = reports_script.run_forced_report(
        business=make_artemea_hito0_business(),
        report_date=date(2026, 5, 19),
        delivery_client=delivery,
        idempotency_store=InMemoryIdempotencyStore(),
        sheets_service_factory=sheets_service_factory,
