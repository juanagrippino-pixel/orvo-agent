...
def test_force_report_merges_all_enabled_hito0_connectors_before_dispatching_once():
    delivery = MagicMock()
    delivery.send_text.return_value = DeliveryResult(success=True, message_id="dry-run", error=None)
    sheets_service_factory = MagicMock(side_effect=AssertionError("google sheets should not be loaded for tiendanube/meta"))
    ...
    sent_text = delivery.send_text.call_args.args[1]
    assert "Ventas" in sent_text
    assert "Meta Ads ARTEMEA" in sent_text
    sheets_service_factory.assert_not_called()
