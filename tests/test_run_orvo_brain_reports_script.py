...
def test_force_report_runs_all_enabled_hito0_connectors_before_dispatching_once():
    """Forced dry-runs must preview the same TN + Meta payload as scheduled Hito 0 sends."""
    ...
    assert {ev.source for ev in metrics["revenue_today"].evidence} == {"tiendanube"}
    assert {ev.source for ev in metrics["ad_spend_today"].evidence} == {"meta_ads"}
    assert result.dispatch.status == "sent"
